	...
	else
	{
		switch ($FIELD)
		{
			...
			case "CONFIRM_PASSWORD":
				?><input type="password" id="probel_PASSWORD" name="REGISTER[<?=$FIELD?>]" class="window__input probel_PASSWORD"><?
				break;
...
<!--
value этого поля я хочу использовать здесь  https://xn----8sbkcccbsf1ardia8bhj9u.xn--p1ai/bitrix/admin/fileman_file_edit.php?path=%2Fbitrix%2Fcomponents%2Fcitrus%2Ftszh.confirm_adaptive%2Ftemplates%2F.default%2Fajax%2FconfirmSteps.php&full_src=Y&site=s1&lang=ru&&amp;filter=Y&amp;set_filter=Y
-->




<!-- Номер этот лицевого счета нужно передать -->
<div class="window__block">
	<div class="window__input-name">Ваш лицевой счет:<span class="window__input-star">*</span>
	</div>
	<input class="window__input" type="text" maxlength="6" name="REGISTER[UF_LIC]" value="" placeholder="Лицевой счет из квитанции">
	<div class="block-register-form__note"><a href="https://xn----8sbkcccbsf1ardia8bhj9u.xn--p1ai/img/Screenshot_2.png" target="_blank"><span style="color:red;font-weight:bold">Как узнать номер лицевого счета?</span></a></div>
</div>
...
<script>
    ...
    $(document).ready(function () {
		...
		$("#probel_LOGIN").change(function() {
    	var re=/[ а-яА-ЯёЁ]/;
    	var val=$("#probel_LOGIN").val().replace(re, '');
    	$("#probel_LOGIN").val(val);
	});

		$("#probel_LOGIN").keyup(function() {
    	$("#probel_LOGIN").change();
	});

		$(".probel_PASSWORD").change(function() {
    	var re=/[ а-яА-ЯёЁ]/;
    	var val=$(".probel_PASSWORD").val().replace(re, '');
    	$(".probel_PASSWORD").val(val);
	});

		$(".probel_PASSWORD").keyup(function() {
    	$(".probel_PASSWORD").change();
	});
