	...
	else
	{
		switch ($FIELD)
		{
			...
			case "WORK_NOTES":
				...
				break;
                        case "PERSONAL_PHONE":
                            ...
                            <input type="text" id="customer_phone"  value="<?=$arrResult["VALUES"][$FIELD]?>" class="window__input"><br>
			...
			default:
				...
				?><input class="window__input" type="text" maxlength="50"  name="REGISTER[<?=$FIELD?>]" value="<?=$arrResult["VALUES"][$FIELD]?>" placeholder="<?=GetMessage("REGISTER_FIELD_" . $FIELD)?>"><?
...
<form class="block-register-form" name="block-register-form<?= $arResult["RND"] ?>" method="post" target="_top" action="/?login=yes">
    ...
    <div class="window__register_field">
        ...
        <div class="window__block">
            ...
            <input class="window__button" type="submit" onclick="getValuePhone()" value="<?= getMessage("AUTH_REGISTER_BUTTON") ?>">
        </div>
    </div>
